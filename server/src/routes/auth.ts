...
router.get(
  ...
  async (req: AuthenticatedRequest, res) => {
    ...
    try {
      ...
      if (email) {
        updatedIntegrations.google_sheets.email = email;
      }
      ...
      //   google_sheet_email: robot.integrations?.google_sheets?.email,
      //   jwtToken,
      //   files,
      // });
...
router.post(
  ...
  async (req: AuthenticatedRequest, res) => {
    ...
    oauth2Client.setCredentials({
      access_token: robot.integrations?.google_sheets?.access_token,
      refresh_token: robot.integrations?.google_sheets?.refresh_token,
    });
...
router.post("/gsheets/update", requireSignIn, async (req, res) => {
  ...
  try {
    ...
    if (spreadsheetId) {
      updatedIntegrations.google_sheets.sheet_id = spreadsheetId;
    }
    if (spreadsheetName) {
      updatedIntegrations.google_sheets.sheet_name = spreadsheetName;
    }
...
router.post(
  ...
  async (req: AuthenticatedRequest, res) => {
    ...
    try {
      ...
      // Ensure google_sheets is initialized as an object if it doesn't exist
      updatedIntegrations.google_sheets = {
        email: null,
        sheet_id: null,
        sheet_name: null,
        access_token: null,
